...
class ResnetGenerator(nn.Module):
    ...
    def _get_linear_weight(self, module):
        if hasattr(module, 'weight') and isinstance(module.weight, torch.Tensor):
            return module.weight
        if hasattr(module, 'weight_orig') and isinstance(module.weight_orig, torch.Tensor):
            return module.weight_orig
    ...
    def forward(self, input):
        ...
        gap_weight = self._get_linear_weight(self.gap_fc)
        if gap_weight is None:
            raise RuntimeError(
                f"gap_fc has no accessible weight parameter: {self.gap_fc}")
        gap = x * gap_weight.unsqueeze(2).unsqueeze(3)

        gmp = torch.nn.functional.adaptive_max_pool2d(x, 1)
        gmp_logit = self.gmp_fc(gmp.view(x.shape[0], -1))
        gmp_weight = self._get_linear_weight(self.gmp_fc)
        if gmp_weight is None:
            raise RuntimeError(
                f"gmp_fc has no accessible weight parameter: {self.gmp_fc}")
