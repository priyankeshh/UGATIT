...
def validate(model, val_loader, step, kid_subset_size=100):
    ...
    vis_real_A, vis_real_B, vis_fake_B, vis_fake_A = None, None, None, None
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            # Forward pass
            d_loss, g_loss, fake_B = model(real_A, real_B)

            # Compute fake_A as well for visualization
            fake_A = model.module.genB2A(real_B)

            val_disc_loss += d_loss.mean().item()
            val_gen_loss += g_loss.mean().item()
            num_batches += 1

            # Collect images for KID (subset)
            ...
            # Collect first 2 images for visualization
    ...
    if vis_real_A is not None:
        grid_A = make_grid(
            torch.cat([vis_real_A, vis_fake_B], dim=0), nrow=2, normalize=True)
        grid_B = make_grid(
            torch.cat([vis_real_B, vis_fake_A], dim=0), nrow=2, normalize=True)
        image_grids = {
            'RealA_FakeB': grid_A,
            'RealB_FakeA': grid_B
        }
